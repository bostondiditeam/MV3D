...
{   
    ...
    if(meas_package.sensor_type_== MeasurementPackage::LASER){
        ...
        px = (meas_package.raw_measurements_(0) + 2.0)* cos(meas_package.raw_measurements_(1)) + 1.5;
        py = -(meas_package.raw_measurements_(0) + 2.0)* sin(meas_package.raw_measurements_(1));
        x_ <<   px,
                py,
                0,
                0,
...
void classA::passSigmaPointsToProcessFunc(MatrixXd Xsig_aug, const void * timeDiff)
{
    ...
    double delta_t = *indata/1000000000.0;
...
{   
    ...
    for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //2n+1 simga points
        ...
        Zsig(1,i) = atan2(-p_y,p_x-1.5);                            //phi
        Zsig(2,i) = ((p_x-1.5)*v1 + p_y*v2 ) / (sqrt(p_x*p_x + p_y*p_y)+2.0);   //r_dot
    }
