...
class Tracklet(object):
    ...
    num_frames = None
    scores = None  # float len-n score
    bboxes = None # nx(8x3) array
                ...
                if info.tag == 'objectType':
                    ...
                    for pose in info:
                        if pose.tag == 'count':  # this should come before the others
                            ...
                            new_track.scores = np.nan * np.ones(new_track.num_frames, dtype=float)
                            new_track.bboxes = np.nan * np.ones((new_track.num_frames, 8, 3), dtype=float)
                            frame_idx = 0
                            ...
                            for poseInfo in pose:
                                if poseInfo.tag == 'tx':
                                    ...
                                    new_track.scores[frame_idx] = float(poseInfo.text)  
                                elif poseInfo.tag == 'bbox':
                                    pass
                                    #bbox_str = re.sub('[^0-9 .-]','', poseInfo.text)
                                    #print(bbox_str, np.fromstring(bbox_str, sep=' ').shape)
                                    #new_track.bboxes[frame_idx] = np.fromstring(bbox_str, sep=' ').reshape(-1,8,3)  
                                else:
