...
class Projection:
    ...
    def __init__(self, bag_file, md_path, calib_file, tracklets, tracklets_corrected):
        self.timestamp_map = extract_bag_timestamps(bag_file)
        self.calib_file = calib_file
        self.frame_map = generate_frame_map(tracklets)
        self.frame_map_corrected = generate_frame_map(tracklets_corrected)
        self.offset = None
...
if __name__ == "__main__" :
    ...
    parser.add_argument('tracklet_corrected', type=str, nargs='?', default='', help='corrected tracklet filename')
    ...
    tracklet_file_corrected = args.tracklet_corrected
    ...
    tracklets_corrected = parse_xml(tracklet_file_corrected)

    try :
        p = Projection(bag_file, md_path, calib_file, tracklets, tracklets_corrected)
        if args.offset :
