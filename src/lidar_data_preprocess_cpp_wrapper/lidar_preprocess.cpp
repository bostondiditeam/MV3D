...
int X_SIZE = (int)((x_MAX-x_MIN)/x_DIVISION);	//meter/meter = grid #
int Y_SIZE = (int)((y_MAX-y_MIN)/y_DIVISION);	//meter/meter = grid #
int Z_SIZE = (int)((z_MAX-z_MIN)/z_DIVISION);	//meter/meter = grid #
...
PyObject* lidar_preprocess(const char* str)//int argc, char** argv)
{
	PyObject* result_obj = PyList_New(0);
...
//		std::cout <<"intensity_min:"<<intensity_MIN <<", intensity_max:"<<intensity_MAX<<std::endl;
	...
	PyObject* density_map_obj = PyList_New(0);
	PyObject* intensity_map_obj = PyList_New(0);
	for (int Y=0; Y<Y_SIZE; Y++)
	{
		PyObject* density_x = PyList_New(0);
		PyObject* intensity_x = PyList_New(0);
		for (int X=0; X<X_SIZE; X++)
		{
			...
			PyObject* density_rgb = PyList_New(0);
			PyList_Append(density_rgb, PyInt_FromLong( (int)((density_map[X][Y]-density_MIN)/density_MAX *255) ));
			PyList_Append(density_rgb, PyInt_FromLong( (int)((density_map[X][Y]-density_MIN)/density_MAX *255) ));
			PyList_Append(density_rgb, PyInt_FromLong( (int)((density_map[X][Y]-density_MIN)/density_MAX *255) ));
			PyList_Append(density_x, density_rgb);

			//method 2 (faster):
//			PyList_Append(density_map_obj, PyInt_FromLong( (int)((density_map[X][Y]-density_MIN)/density_MAX *255) ));
//			PyList_Append(density_map_obj, PyInt_FromLong( (int)((density_map[X][Y]-density_MIN)/density_MAX *255) ));
//			PyList_Append(density_map_obj, PyInt_FromLong( (int)((density_map[X][Y]-density_MIN)/density_MAX *255) ));
			...
			PyObject* intensity_rgb = PyList_New(0);
			PyList_Append(intensity_rgb, PyInt_FromLong( (int)((intensity_map[X][Y]-intensity_MIN)/intensity_MAX *255) ));
			PyList_Append(intensity_rgb, PyInt_FromLong( (int)((intensity_map[X][Y]-intensity_MIN)/intensity_MAX *255) ));
			PyList_Append(intensity_rgb, PyInt_FromLong( (int)((intensity_map[X][Y]-intensity_MIN)/intensity_MAX *255) ));
			PyList_Append(intensity_x, intensity_rgb);

			//method 2 (faster):
//			PyList_Append(intensity_map_obj, PyInt_FromLong( (int)((intensity_map[X][Y]-intensity_MIN)/intensity_MAX *255) ));	
//			PyList_Append(intensity_map_obj, PyInt_FromLong( (int)((intensity_map[X][Y]-intensity_MIN)/intensity_MAX *255) ));	
//			PyList_Append(intensity_map_obj, PyInt_FromLong( (int)((intensity_map[X][Y]-intensity_MIN)/intensity_MAX *255) ));			
		}	
		PyList_Append(density_map_obj, density_x);
		PyList_Append(intensity_map_obj, intensity_x);
	}
	PyList_Append(result_obj, density_map_obj);
	PyList_Append(result_obj, intensity_map_obj);
...
//			std::cout<<"Z:"<<Z<<", "<<"height_MIN[Z]:"<<height_MIN[Z]<<", height_MAX[Z]:"<<height_MAX[Z]<<std::endl;
	...
	//std::cout << "Z SIZE:" << Z_SIZE <<std::endl;
	//std::cout << "Y SIZE:" << Y_SIZE <<std::endl;
	//std::cout << "X_SIZE:" << X_SIZE <<std::endl;
	//std::cout << "Z_SIZE * Y_SIZE * X_SIZE * 3 (RGB):" << Z_SIZE*Y_SIZE*X_SIZE*3 <<std::endl;

	PyObject* height_maps_obj = PyList_New(0);
	for (int Z=0; Z<Z_SIZE; Z++)
	{
		PyObject* height_y = PyList_New(0);
		for (int Y=0; Y<Y_SIZE; Y++)
		{
			PyObject* height_x = PyList_New(0);
			for (int X=0; X<X_SIZE; X++)
			{
				...
				PyObject* height_rgb = PyList_New(0);
				PyList_Append(height_rgb, PyInt_FromLong( (int)((height_maps[X][Y][Z]-height_MIN[Z])/height_MAX[Z] *255) ));
				PyList_Append(height_rgb, PyInt_FromLong( (int)((height_maps[X][Y][Z]-height_MIN[Z])/height_MAX[Z] *255) ));
				PyList_Append(height_rgb, PyInt_FromLong( (int)((height_maps[X][Y][Z]-height_MIN[Z])/height_MAX[Z] *255) ));
				PyList_Append(height_x, height_rgb);

				//method 2 (faster):
				//PyList_Append(height_maps_obj, PyInt_FromLong( (int)((height_maps[X][Y][Z]-height_MIN[Z])/height_MAX[Z] *255) ));
				//PyList_Append(height_maps_obj, PyInt_FromLong( (int)((height_maps[X][Y][Z]-height_MIN[Z])/height_MAX[Z] *255) ));
				//PyList_Append(height_maps_obj, PyInt_FromLong( (int)((height_maps[X][Y][Z]-height_MIN[Z])/height_MAX[Z] *255) ));
			}
			PyList_Append(height_y, height_x);
		}
		PyList_Append(height_maps_obj,height_y);
	...
	PyList_Append(result_obj, height_maps_obj);
	return result_obj;
